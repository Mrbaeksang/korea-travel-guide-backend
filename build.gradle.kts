...
buildConfig {
    ...
    // 새로운 타입 설명 방식: 타입 이름만 제공 (코드는 백엔드에서 자동 매핑)
    val contentTypeDescription =
        file("src/main/resources/content-type-id.yml")
            .readText()
            .substringAfter("types-description:")
            .substringBefore("\n")
            .trim()
            .replace("\"", "")
    ...
    buildConfigField("String", "CONTENT_TYPE_DESCRIPTION", "\"\"\"$contentTypeDescription\"\"\"")
