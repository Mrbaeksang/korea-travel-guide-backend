...
class TourApiClient(
    ...
    fun fetchFestivalInfo(
        params: TourFestivalParams,
        languageCode: String,
    ): TourResponse {
        val url =
            buildTourUri(languageCode, "searchFestival2") {
                queryParam("eventStartDate", params.eventStartDate)
                params.eventEndDate?.let { queryParam("eventEndDate", it) }
                params.areaCode?.let { queryParam("areaCode", it) }
                params.sigunguCode?.let { queryParam("sigunguCode", it) }
            }

        val body =
            runCatching { restTemplate.getForObject(url, String::class.java) }
                .onFailure { log.error("축제 정보 조회 실패 - languageCode={}", languageCode, it) }
                .getOrNull()
    ...
    fun fetchStayInfo(
        params: TourStayParams,
        languageCode: String,
    ): TourResponse {
        val url =
            buildTourUri(languageCode, "searchStay2") {
                params.areaCode?.let { queryParam("areaCode", it) }
                params.sigunguCode?.let { queryParam("sigunguCode", it) }
                params.modifiedtime?.let { queryParam("modifiedtime", it) }
                params.arrange?.let { queryParam("arrange", it) }
            }

        val body =
            runCatching { restTemplate.getForObject(url, String::class.java) }
                .onFailure { log.error("숙박 정보 조회 실패 - languageCode={}", languageCode, it) }
                .getOrNull()
